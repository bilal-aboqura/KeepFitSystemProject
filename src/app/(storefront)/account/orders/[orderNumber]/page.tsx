...
export default async function AccountOrderPage({params}:{params:Promise<{orderNumber:string}>}) {
  ...
  return <section className="glass-elevated break-words p-6"><Link href="/account/orders" className="underline">{t.back}</Link>
    ...
    <ul className="mt-6 space-y-3">{o.order_items.map(i=><li key={i.id} className="flex flex-wrap justify-between gap-2"><span>{ar?(i.name_ar||i.name_en):i.name_en}{i.unit_label_en ? ` · ${ar ? i.unit_label_ar : i.unit_label_en}` : ""} × {i.quantity}</span><span>{formatPrice(Number(i.line_total ?? Number(i.price)*i.quantity),lang)}</span></li>)}</ul>
